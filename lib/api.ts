...
export const api = {
  ...
  createOrder: async (orderData: any): Promise<{ id: number; success: boolean; payment_url?: string }> => {
    ...
    // 2. ATTEMPT SECURE PROXY (Priority)
    // This talks to the custom PHP code in functions.php which handles Order + UddoktaPay Link Generation
    try {
        ...
        if (response.ok) {
            ...
            console.warn("Secure Proxy returned error, falling back...");
        }
    } catch (proxyError) {
        console.warn("Secure Proxy Failed, using insecure fallback", proxyError);
    }

    // 3. FALLBACK: USE DIRECT API (Requires Write Keys in Frontend)
    // This path executes if you forgot to add the PHP code or if the Proxy crashes.
    try {
        ...
        if (orderData.payment_method === 'uddoktapay') {
             // Fallback: Construct Official Pay Link which is crash-proof
