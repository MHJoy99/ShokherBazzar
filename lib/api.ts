...
// Once you add the PHP Proxy code, you can downgrade these keys to "Read Only" in WordPress
...
export const api = {
  ...
  createOrder: async (orderData: any): Promise<{ id: number; success: boolean; payment_url?: string }> => {
    ...
    // 2. ATTEMPT SECURE PROXY (Does not require Write keys)
    try {
        ...
        if (response.ok) {
            ...
            return {
                id: data.id,
                success: true,
                payment_url: data.payment_url // The proxy returns the URL generated by the PHP hook
            };
        } else {
            throw new Error("Secure Proxy Failed");
        }
    } catch (proxyError) {
        // 3. FALLBACK: USE LEGACY DIRECT API (Only works if Frontend Keys have Write Access)
        // This is kept as backup if you forget to add the PHP code.
        console.warn("Using insecure fallback for order creation", proxyError);
        const order = await fetchWooCommerce('/orders', 'POST', { ...payload, set_paid: false });
        
        let payment_url = null;
        
        // Manual Link Construction fallback for fallback
        if (orderData.payment_method === 'uddoktapay') {
             // Fallback 1: Check meta
             const metaUrl = order.meta_data.find((m: any) => m.key === '_api_payment_url');
             if (metaUrl) payment_url = metaUrl.value;
             
             // Fallback 2: Construct Official Pay Link
             if (!payment_url) {
